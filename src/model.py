...
class CatMultiTaskClassifier(BaseClassifier):
    ...
    def __init__(
        ...
        if shared_features:
            ...
            self.breed_head = nn.Sequential(nn.GELU(), nn.Linear(feature_dim, num_breed_classes))
            self.emotion_head = nn.Sequential(
                nn.GELU(), nn.Linear(feature_dim, num_emotion_classes)
            )
        else:
    ...
    def forward(
        self, x: torch.Tensor, task: str = "both"
        ...
        Args:
            x: Input tensor
            task: Which task to compute ("breed", "emotion", or "both")

        Returns:
            Task logits or tuple of (breed_logits, emotion_logits) if task="both"
        """
        features = self.backbone(x)
        ...
        if self.shared_features:
            ...
            breed_features = self.breed_feature_extractor(features)
            emotion_features = self.emotion_feature_extractor(features)
            breed_logits = self.breed_head(breed_features)
            emotion_logits = self.emotion_head(emotion_features)

        return {
            "breed": breed_logits,
            "emotion": emotion_logits,
            "both": (breed_logits, emotion_logits),
        }.get(task, (breed_logits, emotion_logits))
...
def load_model(
    ...
    if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
        ...
        model = checkpoint
        logger.info(f"Loaded complete {model_type} model from {path.relative_to(Path.cwd())}")
...
def save_model_summary(
    ...
    try:
        ...
        logger.info(f"Model architecture saved to {save_path.relative_to(Path.cwd())}")
        ...
        try:
            with save_path.with_suffix(".txt").open("w") as f:
                f.write(str(model))
            logger.info(
                f"Model summary saved as text to {save_path.with_suffix('.txt').relative_to(Path.cwd())}"
            )
